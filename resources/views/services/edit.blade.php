    ...
    <section class="bg-white dark:bg-gray-900">
        ...
        <nav class="flex px-10" aria-label="Breadcrumb">
            ...
            <li aria-current="page">
                ...
                <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">Editer le service</span>
                </div>
            </li>
            </ol>
        </nav>
                    ...
                    <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                        <div class="sm:col-span-2">
                            <label for="designation" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white uppercase font-roboto">Designation</label>
                             <input value="{{old('designation',$service->nom)}}" type="text" name="designation" id="designation" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"  required="" value="{{old('designation')}}">
                             @error('designation') <span class="text-red-600 font-roboto">{{ $message }}</span> @enderror
                            </div>
                            <div>
                                <label for="description" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Description </label>
                                <textarea  required   type="description" name="description" id="description" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"   cols="100" rows="6">{{old('description',$service->description)}}</textarea>
